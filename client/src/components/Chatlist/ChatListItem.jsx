...
export default function ChatListItem({ data, isContactsPage = false }) {
  ...
  const [totalUnreadMessages,setTotalUnreadMessages] = useState(data?.totalUnreadMessages)
      ...
      <div className=" min-h-full flex flex-col justify-center mt-3 pr-2 w-full">
        ...
        <div className="flex border-b border-conversation-border pb-2 pt-1 pr-2">
          <div className=" flex justify-between w-full">
            <span className=" text-secondary line-clamp-1 text-sm">
              {isContactsPage ? (
                ...
                <div className="flex items-center gap-1 max-w-[200px] sm:max-w-[250px] md:max-w-[300px] lg:max-w-[200px] xl:max-w-[300px]">
                  ...
                  {data.type === "text" && (
                    <span className=" truncate">{decryptData(data.message)}</span>
                  )}
                  {data.type === "audio" && (
            ...
            {
              totalUnreadMessages>0 && <span className=" bg-icon-green px-[5px] rounded-full text-sm">{data.totalUnreadMessages}</span>
            }
          </div>
        </div>
      </div>
