...
export default function SearchMessages() {
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      setSearchMessages(
        messages.filter(
          (message) =>
            message.type === "text" && decryptData(message.message).includes(searchTerm)
        )
      );
    } else {
      setSearchMessages([]);
  ...
  return (
    <div className=" border-conversation-border border-l w-full bg-conversation-panel-background flex flex-col z-10  md:max-h-screen">
          ...
          <div className=" flex flex-col w-full h-full">
            {searchMessages.map((message) => (
              <div className=" flex cursor-pointer flex-col justify-center hover:bg-background-default-hover w-full px-5 border-b-[0.1px] border-secondary py-5">
                ...
                <div className=" text-icon-green">{decryptData(message.message)}</div>
              </div>
            ))}
          </div>
        </div>
